...
def double_operator(chars):
    for char in range(len(chars)-1):
        return chars[char] in ['+', '-', '*', '/', '//', '%'] and chars[char+1] in ['*', '/', '//', '%'] or \
        chars[char] in ['*', '/', '//', '%'] and chars[char+1] in ['+', '-', '*', '/', '//', '%']

def no_operator_between_numbers(chars):
    while ' ' in chars:
        chars=chars.replace(' ','')
    if all(chars[char] in '0123456789' and chars[char+1] == ' ' and chars[char+2] in '0123456789' for char in range(len(chars) - 2)):
        return True
    else: return False
